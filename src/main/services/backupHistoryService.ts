...
export class BackupHistoryService {
  ...
  addBackupEntry(
    ...
    try {
      if (!fs.existsSync(backupPath)) {
        this.logger.warn(`Cannot add backup to history: file does not exist: ${backupPath}`);
        return;
      }
      ...
      this.logger.info(`Added backup to history: ${fileName} (${backupItem.sizeFormatted})`);
  ...
  addFailedBackupEntry(
    ...
    try {
      ...
      this.logger.error(`Failed to add failed backup to history: ${error}`);
    }
  }
      ...
      for (const fileName of backupFiles) {
        ...
        try {
          ...
          if (storedItem && storedItem.status === 'completed') {
            ...
            this.logger.debug(`Discovered untracked backup file: ${fileName}`);
          }
        } catch (error) {
          this.logger.warn(`Failed to read backup file stats: ${fileName} - ${error}`);
        }
      }

      // Add failed backups from stored history (they have no files)
      const failedBackups = storedHistory.filter(item => item.status === 'failed');
      discoveredBackups.push(...failedBackups);

      // Sort by date (newest first)
      discoveredBackups.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());

      // Update the stored history with discovered backups
      this.store.set('backupHistory', discoveredBackups);

      this.logger.debug(`Discovered ${backupFiles.length} backup files in ${backupDir}`);
      return discoveredBackups;

    } catch (error) {
      this.logger.error(`Failed to scan backup directory: ${error}`);
      // Return stored history as fallback
      return storedHistory.filter(item => item.status === 'failed' || (item.path && fs.existsSync(item.path)));
    }
  }
